...
export function buildPageGenerationPrompt(
  ...
  sections.push(
    ...
    ``,
    `### Hero Headline Quality (CRITICAL — STRICT ENFORCEMENT)`,
    `- ABSOLUTELY FORBIDDEN in the hero heading_text: "Welcome", "Welcome to [anything]", "Home", "About Us", "About Our Company", "Our Services", or ANY generic/boilerplate greeting`,
    `- If the content plan's hero heading contains "Welcome" or any forbidden phrase, you MUST REPLACE it with a marketing-grade alternative — do NOT copy it verbatim`,
    `- The hero h1 heading_text MUST be a compelling, marketing-grade headline that communicates the business's core value proposition`,
    `- Draw directly from the Site Context above: use key messages, differentiators, industry terms, and audience pain points to craft the headline`,
    `- Include the primary SEO keyword naturally in the hero heading_text`,
    ...
    `- Bad examples: "Welcome to Smith Dental", "Welcome", "About Our Company", "Home Page", "Discover Our Services"`,
    `- The hero sub_headline or supporting text should expand on the h1 with a specific benefit or call to action`,
    ``,
    `### Content Contextuality (ALL text props — CRITICAL)`,
    `- EVERY text value in props_json (heading_text, text, description, label, etc.) MUST be contextual to the business details from the Site Context above`,
    `- Reference the business name, industry, services, audience, differentiators, and key messages — not generic placeholder language`,
    `- Generic phrases like "We offer quality services", "Our team is dedicated", "Get in touch today" are FORBIDDEN — always be specific to THIS business`,
    ``,
    `### Icon Validation`,
