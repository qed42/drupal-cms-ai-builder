...
export function buildPageGenerationPrompt(
  ...
  sections.push(
    ...
    `     - "component_id": organism component ID from the Space DS manifest (e.g., "space_ds:space-hero-banner-style-02", "space_ds:space-cta-banner-type-1")`,
    ...
    `       - "component_id": atom/molecule component ID from the Space DS manifest (e.g., "space_ds:space-heading", "space_ds:space-text")`,
    ...
    `- Width is controlled at the container level, NOT on individual components. Do NOT add "width" to component props_json unless it is explicitly listed in the Component Prop Reference above (only CTA Banner has its own width prop).`,
    ...
    `- **Space Text**: The "rich_text" value must end with a newline character.`,
    `- **Space Heading**: Always set "alignment" prop (use "none" if no preference).`,
